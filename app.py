...
def load_data():
    ...
    # 3. Skor Komposit Produktivitas (rata-rata kolom 9-12)
...
with tab3:
    ...
    with col_h:
        ...
        if 1 in prob_bersyarat.index and 'Fokus Tinggi (%)' in prob_bersyarat.columns:
            p_kopi = prob_bersyarat.loc[1, 'Fokus Tinggi (%)'] if 1 in df_filtered['Is_Peminum_Kopi'].values else 0
            p_non_kopi = prob_bersyarat.loc[0, 'Fokus Tinggi (%)'] if 0 in df_filtered['Is_Peminum_Kopi'].values else 0
        else:
            p_kopi, p_non_kopi = 55.0, 27.27
...
with tab4:
    ...
    with col_j:
        ...
        if 'mc_results' in st.session_state:
            ...
            fig_mc.add_vline(x=mean_mc, line_dash="dash", line_color="blue", 
                           annotation_text=f"Mean: {mean_mc:.3f}")
            fig_mc.add_vline(x=ci_bawah, line_dash="dot", line_color="purple",
                           annotation_text=f"CI Bawah: {ci_bawah:.3f}")
            fig_mc.add_vline(x=ci_atas, line_dash="dot", line_color="purple",
